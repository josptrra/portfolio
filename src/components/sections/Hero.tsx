...
export function Hero() {
  ...
  return (
    <section className="min-h-screen flex flex-col justify-center px-8 relative max-w-4xl mx-auto pt-20">
        ...
        {(step >= 1 || isMobile) && (
          <h1 className="text-4xl md:text-6xl font-display font-bold text-accent text-glow mb-4">
            {isMobile ? "JULIO SYAH PUTRA" : (
              <TypeWriter text="JULIO SYAH PUTRA" speed={50} showCursor={false} onComplete={() => setStep(2)} />
            )}
          </h1>
        )}
        ...
        {(step >= 2 || isMobile) && (
          <p className="text-xl md:text-2xl text-text font-sans mb-6 h-8">
            {isMobile ? t('hero', 'subtitle') : (
              <TypeWriter text={t('hero', 'subtitle')} speed={30} showCursor={false} onComplete={() => setStep(3)} />
            )}
          </p>
        )}
